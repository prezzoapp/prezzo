...
export default class ItemImagePicker extends Component {
    ...
    render() {
        return (
            <View>
                {this.props.editable &&
                    <TouchableOpacity style={styles.closeBtn} activeOpacity={0.6} onPress={() => this.props.deleteImage(this.props.image.id)}>
                        <Icon title="Delete" name="md-close" color="black" size={14} style={styles.closeBtnIcon} />
                    </TouchableOpacity>
                }

                {
                    (this.props.image.image_path == '' || this.props.image.image_path == undefined)
                        ?
